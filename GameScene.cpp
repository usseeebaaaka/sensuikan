...
void GameScene::update(float dt) {
	...
	for (b2Body* b = world->GetBodyList(); b; b = b->GetNext()) {
		...
		if (objectTag == kTag_Call_Scroll) {
			...
			} if(myUnit->getPositionY() < getWindowSize().height * 0.3) {
...
void GameScene::createMissileLeft(b2Vec2 position) {
	...
	missileBody->SetLinearVelocity(b2Vec2((-1.5 * PI - playerAngle * 3) / 2, (-playerAngle * 3) / 2));										// x座標y座標に圧力をかける
}
...
void GameScene::createMissileDiagonal(b2Vec2 position) {
	...
	missileBody->SetLinearVelocity(b2Vec2(-playerAngle, (PI -playerAngle * 2) / 2));										// x座標y座標に圧力をかける
}
/*

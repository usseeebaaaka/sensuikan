...
void GameScene::hitPlayer () {
	...
	unitData[kTag_PlayerUnit]->setHp(unitData[kTag_PlayerUnit]->getHp() - 1);	// hpを-1してセット
...
void GameScene::hitSubmarine() {
	unitData[kTag_EnemySubmarine]->setHp(unitData[kTag_EnemySubmarine]->getHp() - 1);		// 敵潜水艦のhpを-1する
	CCSprite* bombAction = CCSprite::create("hit0.png");							// hit0.pngを取得
	bombAction->setPosition(unitData[kTag_EnemySubmarine]->getPosition());			// 敵潜水艦のオブジェクト(潜水艦)と同じ座標にセット
...
void GameScene::update(float dt) {
	...
	for (b2Body* b = world->GetBodyList(); b; b = b->GetNext()) {
		...
		if (objectTag == kTag_Call_Scroll) {
			...
			// ミサイルと駆逐艦が衝突した場合
		} else if (objectTag == kTag_CollisionDestroyer) {
			score_and_Maxplace += 10;									// スコアを10加算
			setScoreNumber();											// スコアを更新して表示
			removeObject(object, (void*)b);								// ミサイルを除去
			hitSubmarine();												// 敵機のhpを-1する、0になった場合除去
